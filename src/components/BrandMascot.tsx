...
export function BrandMascot({ size = 120, className = "", variant = "default" }: BrandMascotProps) {
  ...
  // Pupil base positions (center of eye whites)
  const leftPupilCx = leftEyeCx + pupilOffset.x;
  const leftPupilCy = leftEyeCy + pupilOffset.y;
  const rightPupilCx = rightEyeCx + pupilOffset.x;
  const rightPupilCy = rightEyeCy + pupilOffset.y;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tail */}
        <motion.g style={{ originX: `${cx + bodyR * 0.6}px`, originY: `${cy + bodyR * 0.3}px` }} animate={tailWag}>
          <path
